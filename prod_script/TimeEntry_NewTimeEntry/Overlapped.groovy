...
Map<String, String> countParam = Map.of("tenantId",user.get("tenantId").asLong(),"userId",user.get("tenantUserId").asLong(),"ruleId",event.getId(),"escalation",Boolean.parseBoolean(params.get("escalation").asText()))

println event.getId()+"Overlapped"
          ...
          println ZonedDateTime.parse(i.get("endDate").asText()).toLocalDateTime().plusHours(Integer.parseInt(user.get("tz").asText().substring(0,3))).format(DateTimeFormatter.ofPattern("MM/dd HH:mm"))+" "+user.get("tzStr").asText()
            ...
            if(Integer.parseInt(api.call("mapbots-core","/teams/messages/countMessagesByForRule",ruleParams).asText()) == 0){
                ...
                api.call("mspbots-teams", "/message/send", "post",param)
                  Map<String, Object> escalation = Map.of("tenantId",user.get("tenantId").asLong(),"ruleId",event.getId(),"triggerName","Overlapped","times",params.get("times").asText(),"tenantUserId",user.get("tenantUserId").asLong())
