...
if (payload.findPath("status").get("name").asText().toLowerCase().contains("completed")) {
        ...
        if (!resolutionFlg) {
              Map<String, String> param = Map.of("teamsUserId", user.get("teamsUserId").asText(),
                      ...
                      + ", the ticket <a href='https://"+tenant.get("mspbots.sync.wise.site").asText()+"/v4_6_release/ConnectWise.aspx?routeTo=ServiceFV&recid=" + payload.get("id") + "'>" + payload.get("id") + "</a>" + " status is <span style = 'color:#FF0000;font-weight:bold'>\"Completed by Technician\"</span>"
